...
def train(args, data, train_dataloader, dev_dataloader, test_dataloader, rewardModel, actorModel,criticModel, LM_trainable=True,
        ...
        for step, batch in enumerate(epoch_iterator):#进入这个训练数据集中开始遍历
            ...
            if RL_trainable:#如果强化学习
                ...
                for j in range(args.samplecnt):#config里的samplecnt为5，这里是获取action information
                    ...
                    action_list1.append(actions1)#构建Struct-XLM的reward用的
                    action_list2.append(actions2)#构建Struct-XLM的reward用的
                    ...
                    #非强化学习时的loss，随机采样的action的得到的representation
                    # with torch.no_grad():
                ...
                for j in range(args.samplecnt):
                    ...
                    for m in range(true_batch):
                        ...
                        for pos in range(len(actionlist1[j][m])):
                            ...
                            #rr[actionlist1[j][m][pos]] = norm_delta1 * args.alpha#=》SAC-XLM的reward
                            rr[actionlist1[j][m][pos]] = ((losslist1[j][m] - avg1) * args.alpha)#Struct-XLM的reward
                            rr_temp.append(rr)
                        ...
                        for pos in range(len(actionlist2[j][m])):
                            ...
                            #rr[actionlist2[j][m][pos]] = norm_delta2 * args.alpha#SAC-XLM的reward
                            rr[actionlist2[j][m][pos]] = ((losslist2[j][m] - avg2) * args.alpha)#Struct-XLM的reward
                            rr_temp.append(rr)
                    ...
                    if LM_trainable:#训练checkpoint-best的CV
                        with open("src/xtreme_7/output/checkpoint-best-actor-cv-ablationreward.txt", "a") as file:
                        ...
                        with open("src/xtreme_7/output/checkpoint-best-actor-loss-ablationreward.txt", "a") as file:
                            file.write('actor_loss:'+str(actor_loss)+'\n')
                    else:#否则就是训练actor_only时的cv
                        with open("src/xtreme_7/output/checkpoint-only-actor-actor-loss-ablationreward.txt", "a") as file:
                            file.write('actor_loss:'+str(actor_loss)+'\n')
                        with open("src/xtreme_7/output/checkpoint-only-actor-actor-cv-ablationreward.txt", "a") as file:
                            file.write('avg_actor_grad:'+str(avg_actor_grad)+'actor_grad_mean:'+str(actor_grad_mean)+'actor_grad_variance:'+str(actor_grad_var)+'actor_grad_cv:'+str(actor_grad_cv)+'\n') 
                    ...
                    if should_update_critic:
                        ...
                        # 追加内容到文件末尾
                        if LM_trainable:#训练checkpoint-best的CV
                            with open("src/xtreme_7/output/checkpoint-best-critic-cv-ablationreward.txt", "a") as file:
                            ...
                            # 追加内容到文件末尾
                            with open("src/xtreme_7/output/checkpoint-best-critic-loss-ablationreward.txt", "a") as file:
                                file.write('critic_loss:'+str(critic_loss)+'\n')
                        else:#否则就是训练actor_only时的cv
                            with open("src/xtreme_7/output/checkpoint-only-actor-critic-cv-ablationreward.txt", "a") as file:
                                file.write('avg_critic_grad:'+str(avg_critic_grad)+'critic_grad_mean:'+str(critic_grad_mean)+'critic_grad_variance:'+str(critic_grad_var)+'critic_grad_cv:'+str(critic_grad_cv)+'\n') 
                            # 追加内容到文件末尾
                            with open("src/xtreme_7/output/checkpoint-only-actor-critic-loss-ablationreward.txt", "a") as file:
                                file.write('critic_loss:'+str(critic_loss)+'\n')
            ...
            if global_step % args.save_step == 0:#根据config里的信息可知，默认save-step是50步
                ...
                if args.warm_by_action and RL_trainable:
                    ...
                    #收集step和loss
                    with open("src/xtreme_7/output/step_with_loss-ablationreward.txt","a")as file:
    ...
    # 训练全部结束，写入GPU时间日志
    gpu_log_file = os.path.join(args.output_dir, "ablationreward_gpu_time_log.txt")
