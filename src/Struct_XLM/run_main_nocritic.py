...
def train(args, data, train_dataloader, dev_dataloader, test_dataloader, rewardModel, actorModel,criticModel, LM_trainable=True,
    ...
    if RL_trainable:
        ...
        critic_target_optimizer, critic_target_schedule = None, None
        ...
        for step, batch in enumerate(epoch_iterator):#进入这个训练数据集中开始遍历
            ...
            if RL_trainable:#如果强化学习
                ...
                for j in range(args.samplecnt):
                    ...
                    actor_grad1, _, _, actor_loss1, _ = actorModel.get_batch_gradient(
                        context1, mask1, reward_batch=rr_all1, criticmodel=None,update_critic=False)
                    ...
                    actor_grad2, _, _, actor_loss2, _ = actorModel.get_batch_gradient(
                        context2, mask2, reward_batch=rr_all2, criticmodel=None,update_critic=False)
                    critic_grad1 = critic_grad2 = None
                    critic_loss = None
                    # 平均梯度
                    ...
                    if LM_trainable:#训练checkpoint-best的CV
                        with open("src/xtreme_7/output/checkpoint-best-actor-cv-nocritic.txt", "a") as file:
                        ...
                        with open("src/xtreme_7/output/checkpoint-best-actor-loss-nocritic.txt", "a") as file:
                            file.write('actor_loss:'+str(actor_loss)+'\n')
                    else:#否则就是训练actor_only时的cv
                        with open("src/xtreme_7/output/checkpoint-only-actor-actor-loss-nocritic.txt", "a") as file:
                            file.write('actor_loss:'+str(actor_loss)+'\n')
                        with open("src/xtreme_7/output/checkpoint-only-actor-actor-cv-nocritic.txt", "a") as file:
                            file.write('avg_actor_grad:'+str(avg_actor_grad)+'actor_grad_mean:'+str(actor_grad_mean)+'actor_grad_variance:'+str(actor_grad_var)+'actor_grad_cv:'+str(actor_grad_cv)+'\n') 
                    ...
                    if should_update_critic:
                        ...
                        # 追加内容到文件末尾
                        if LM_trainable:#训练checkpoint-best的CV
                            with open("src/xtreme_7/output/checkpoint-best-critic-cv-nocritic.txt", "a") as file:
                            ...
                            # 追加内容到文件末尾
                            with open("src/xtreme_7/output/checkpoint-best-critic-loss-nocritic.txt", "a") as file:
                                file.write('critic_loss:'+str(critic_loss)+'\n')
                        else:#否则就是训练actor_only时的cv
                            with open("src/xtreme_7/output/checkpoint-only-actor-critic-cv-nocritic.txt", "a") as file:
                                file.write('avg_critic_grad:'+str(avg_critic_grad)+'critic_grad_mean:'+str(critic_grad_mean)+'critic_grad_variance:'+str(critic_grad_var)+'critic_grad_cv:'+str(critic_grad_cv)+'\n') 
                            # 追加内容到文件末尾
                            with open("src/xtreme_7/output/checkpoint-only-actor-critic-loss-nocritic.txt", "a") as file:
                                file.write('critic_loss:'+str(critic_loss)+'\n')
            ...
            if global_step % args.save_step == 0:#根据config里的信息可知，默认save-step是50步
                ...
                if args.warm_by_action and RL_trainable:
                    ...
                    #收集step和loss
                    with open("src/xtreme_7/output/step_with_loss-nocritic.txt","a")as file:
                    ...
                    if loss_dev < best_loss:#评价结果达到要求进行模型保存，在这里用loss_dev作为评判标准
                        ...
                        torch.save({}, os.path.join(output_dir, "target_model_critic.bin"))
                        logger.info("w/o Critic: skip saving target_model_critic.bin")
    ...
    # 训练全部结束，写入GPU时间日志
    gpu_log_file = os.path.join(args.output_dir, "nocritic_gpu_time_log.txt")
