...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training, lang2id):
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        try:
            a = encoded_dict['attention_mask']
        except:
            attention_mask = []
            input_list = encoded_dict['input_ids']
            for i in input_list:
                if i != 1:
    ...
    for span in spans:
        ...
        input_ids = np.atleast_1d(span["input_ids"])  # 强制转换为1维数组
        pad_token_indices = np.where(input_ids == tokenizer.pad_token_id)
