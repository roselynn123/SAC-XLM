...
from BertForSequenceClassification_custom import BertForSequenceClassification_custom
...
MODEL_CLASSES = {
    "mbert": (BertConfig, BertForSequenceClassification_custom, BertTokenizer, BertModel),
...
def train(args, train_dataset, model, tokenizer, lang2id=None):
    ...
    for epoch_idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # aveloss_list = [0.0 for _ in range(text.size()[0])]
            ...
            # loss = model(input_ids=text, attention_mask=mask, labels=label,
            #                  action=sentence_actor_attention)[0]
            ...
            if global_step % 2000 == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (args.local_rank == -1 and args.evaluate_during_training):
                        results = evaluate(args, model, tokenizer, split=args.train_split,
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    if args.eval_test_set:
                        ...
                        with open(output_predict_file, 'a') as writer:
                            ...
                            for language in args.predict_languages.split(','):
                                result = evaluate(args, model, tokenizer, split=args.test_split,
                                                  language=language,
                    ...
                    if args.save_only_best_checkpoint:
                        result = evaluate(args, model,tokenizer, split='dev', language=args.train_language,
                        ...
                        if result['acc'] > best_score:
                            ...
                            if args.eval_patience > 0 and patience > args.eval_patience:
                                ...
                                early_stop_output_dir = os.path.join(args.output_dir, "early_stopped_model")
        ...
        #gpu_epoch_times.append(epoch_time_sec)s
...
def write_time_log(output_dir, epoch_times):
    gpu_log_file = os.path.join(output_dir, "XNLI_gpu_time_log.txt")
...
def evaluate(args, model, tokenizer, split='train', language='en', lang2id=None, prefix="",
             output_file=None,
...
def main():
    ...
    # Make sure only the first process in distributed training loads model & vocab处理分布式训练
    if args.local_rank == 0:
        torch.distributed.barrier()
    logger.info("Training/evaluation parameters %s", args)
    # Training
    if args.init_checkpoint:#如果提供了初始化的model，则从指定的检查点加载model
    ...
    else:#否则从args.model_name_or_path中加载预训练model
    ...
    output_dir = os.path.join(args.critic_output_dir, "checkpoint-best_no_policy")#决定使用哪个阶段的模型
    ...
    lora_config =LoraConfig(
        ...
        lora_alpha=4,#缩放因子，会是r的4倍
        target_modules=["query","value","key","dense",
        ],
        lora_dropout=0.1,
        bias="none",
        task_type="SEQ_CLS"#这个部分有问题
    )
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        early_stop_dir = os.path.join(args.output_dir, "early_stopped_model")
        model = model_class.from_pretrained(early_stop_dir,#原本是best_checkpoint
                                            args=args,
                                            config_class=config_class,
                                            model_class=base_model)
        model.to(args.device)
        ...
        with open(output_predict_file, 'a') as writer:
            ...
            for language in args.predict_languages.split(','):
                ...
                result = evaluate(args, model, tokenizer, split=args.test_split, language=language,
                                  lang2id=lang2id,
                                  prefix='early_stop_dir', output_file=output_file, label_list=label_list)
                writer.write('{}={}\n'.format(language, result['acc']))
    ...
    if args.do_predict_dev:
        ...
        with open(output_predict_file, 'w') as writer:
            ...
            for language in args.predict_languages.split(','):
                ...
                result = evaluate(args, model, tokenizer, split='dev', language=language, lang2id=lang2id,
