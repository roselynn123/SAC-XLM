                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.save_only_best_checkpoint:
                        ...
                        if result['acc'] > best_score:
                            ...
                            logger.info("Saving main_optimizer and main_schedule states to %s", output_dir)
                        ...
                        torch.save(main_optimizer.state_dict(), os.path.join(output_dir, "optimizer.pt"))
                        torch.save(main_schedule.state_dict(), os.path.join(output_dir, "scheduler.pt"))
                        logger.info("Saving optimizer and main_schedule states to %s", output_dir)
...
def write_time_log(output_dir, epoch_times):
    gpu_log_file = os.path.join(output_dir, "PAWSX_gpu_time_log.txt")
...
def main():
    ...
    parser.add_argument(
        "--predict_languages", type=str, default="en,de,es,fr,ja,ko,zh", help="prediction languages separated by ','."
    )
    parser.add_argument(
        "--output_dir",
        default='output/pawsx/',
        type=str,
    ...
    output_dir = os.path.join(args.critic_output_dir, "checkpoint-best_actor_only")
    # 定义模型文件路径
    ...
    # Prediction
    #result_file = open("output/pawsx_results.txt", "a+", encoding='utf-8')
    result_file = open("src/xtreme_7/output/pawsx_results.txt", "a+", encoding='utf-8')#打开记录文件
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(best_checkpoint,#best_checkpoint
                                            args=args,
                                            config_class=config_class,
                                            model_class=base_model)
        model.to(args.device)
        ...
        with open(output_predict_file, 'a') as writer:
            ...
            for language in args.predict_languages.split(','):
                ...
                a += "%.1f " % (result['acc'] * 100)
                AVG_F1 += result['acc']
            a += "%.1f " % (AVG_F1 / len(args.predict_languages.split(',')) * 100)
