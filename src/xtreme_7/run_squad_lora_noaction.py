...
def train(args, train_dataset, model, tokenizer):
        ...
        optimizer_grouped_parameters = [
            ...
            },
            {
                "params": [p for n, p in model.named_parameters() if
                        not any(nd in n for nd in other) and any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
    ...
    set_seed(args)
    best_score = 0
    #for epoc_num in train_iterator:
    for epoch_idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     constituent_num_total = 0
            #     actions,sentence_actor_attention,_ =Sampling_RL_batch(args,predicted,inputs["attention_mask"],head_mask,args.epsilon, Random=False)#这个部分的mask可能会需要修改
...
def write_time_log(output_dir, epoch_times):
    gpu_log_file = os.path.join(output_dir, "XQUAD_gpu_time_log.txt")
...
def main():
    ...
    parser.add_argument("--test_file", default="xquad, mlqa", type=str)  # "mlqa", "xquad", "tydiqa"
    parser.add_argument(
    ...
    parser.add_argument("--critic_output_dir", default="/data/home10b/wlj/code/Struct-XLM/src/Struct_XLM/output_lm/checkpoint-best_no_policy", type=str)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(early_stop_dir, force_download=True,
                                            args=args,
                                            config_class=config_class,
                                            model_class=base_model, )
        ...
        if "xquad" in args.test_file:
            ...
            for lang in ["ar", "de", "el", "en", "es", "hi", "ru", "th", "tr", "vi", "zh"]:
                args.predict_file = u"xquad/xquad.{}.json".format(lang)
                ...
                f1_all += result['f1']
                em_all += result['exact']
            result_str += "{%.1f}" % (f1_all / 11) + "/{%.1f}    " % (em_all / 11)
        ...
        if "mlqa" in args.test_file:
            ...
            for lang in ["ar", "de", "en", "es", "hi", "vi", "zh"]:
                args.predict_file = u"mlqa/MLQA_V1/test/test-context-{}-question-{}.json".format(lang, lang)
                result = evaluate(args, model, tokenizer, prefix=u"mlqa {}".format(lang), language=lang,
                                  lang2id=lang2id, dataset_name="mlqa")
                result = dict((k, v) for k, v in result.items())
                print(result)
                result_str += "{%.1f}" % result['f1'] + "/{%.1f}    " % result['exact_match']
                f1_all += result['f1']
                em_all += result['exact_match']
            result_str += "{%.1f}" % (f1_all / 7) + "/{%.1f}    " % (em_all / 7)
            print(result_str, file=result_file)
        ...
        if "tydiqa" in args.test_file:
            ...
            f1_all = 0
            em_all = 0
            for lang in ["arabic", "bengali", "english", "finnish", "indonesian", "korean", "russian", "swahili", "telugu"]:
