...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
    ...
    for epoch_idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # padding 位置不要产生结构动作
            random_actions = random_actions.masked_fill(mask == 0, -1)
            random_actions[:, 0] = 1#可选，只是将每个句子的第一个token稳定为1

            sentence_actor_attention = sampling_random_batch(
                args,
                mask,
                random_actions
            )
                ...
                if args.save_only_best_checkpoint:
                    ...
                    if result["f1"] > best_score:
                        ...
                        if args.eval_patience > 0 and patience > args.eval_patience:
                            ...
                            #触发早停保存最终完整模型
                            early_stop_output_dir = os.path.join(args.output_dir, "early-stopped-model")
                            
                            if not os.path.exists(early_stop_output_dir):
                                os.makedirs(early_stop_output_dir)
                            model_to_save = model.module if hasattr(model, "module") else model
                            # 1. 检查模型是否为 LoRA 模型
                            if hasattr(model_to_save, 'merge_and_unload'):
                                ...
                                full_model_to_save.save_pretrained(early_stop_output_dir)
                                logger.info("Successfully saved the merged model to %s", early_stop_output_dir)
                            else:
                                # 如果模型不是 LoRA 模型，则按原样保存
                                logger.info("Saving the model without merging LoRA weights (not a PEFT model).")
                                model_to_save.save_pretrained(early_stop_output_dir)
                            tokenizer.save_pretrained(early_stop_output_dir)
...
def write_time_log(output_dir, epoch_times):
    gpu_log_file = os.path.join(output_dir, "NER_gpu_time_log.txt")
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, lang, lang2id=None, few_shot=-1):
    ...
    # 仅在训练模式下使用削减比例来生成独特的缓存文件名
    cache_frac = str(args.train_subset_frac) if mode == 'train' and args.train_subset_frac < 1.0 else "1.0"

    # Load data features from cache or dataset file
    cached_features_file = os.path.join(args.data_dir, "cached_{}_{}_{}_{}_{}".format(mode, lang,cache_frac, # <-- 【新增】抽样比例作为缓存名的一部分
                                                                                   list(filter(None,
    ...
    # ----------------------------------------------------
    # 2. 【新增】实现数据削减逻辑 (应用于 features 列表)
    # ----------------------------------------------------
    if mode == 'train' and args.train_subset_frac < 1.0:
        
        # 依赖于 numpy 在外部脚本中已导入为 np
        import numpy as np
        
        original_size = len(features)
        sample_frac = args.train_subset_frac
        subset_size = int(original_size * sample_frac)
        
        if subset_size == 0 and original_size > 0:
            subset_size = 1 # 确保至少有一个样本
            
        # 随机不重复抽样 indices
        indices = np.random.choice(original_size, subset_size, replace=False)
        
        # 基于抽取的索引选择 features 子集
        features = [features[i] for i in indices]
        
        logger.info("[DATA SUBSET] Original features size: %d", original_size)
        logger.info("[DATA SUBSET] New features size (%.2f%% subset): %d", sample_frac * 100, len(features))
    # ----------------------------------------------------
...
def main():
    ...
    parser.add_argument("--log_file", type=str, default='LORANERlog.txt', help="log file")
    parser.add_argument("--eval_patience", type=int, default=3,
                        help="wait N times of decreasing dev.txt score before early stop during training")
    parser.add_argument("--train_subset_frac", default=1.0, type=float, 
    ...
    output_dir = os.path.join(args.critic_output_dir, "checkpoint-best_no_policy")
    ...
    lora_config =LoraConfig(
        r=16,#越大性能越好，但是速度也会随之减慢
        lora_alpha=32,#缩放因子，会是r的4倍
        target_modules=["query","value","key","dense",],
        lora_dropout=0.1,
        bias="none",
        task_type="TOKEN_CLS"#这个部分需要商讨
    )
    ...
    # 如果参数名包含 lora 或 qa_outputs，则设为可训练
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        early_stop_dir = os.path.join(args.output_dir, "early-stopped-model")#顺利进入early stop后保存的模型
        #logger.info("Loading the best checkpoint from {}\n".format(best_checkpoint))
        logger.info("Loading the best checkpoint from {}\n".format(early_stop_dir))#early_stop_dir
        #tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        tokenizer = tokenizer_class.from_pretrained(early_stop_dir, do_lower_case=args.do_lower_case)#顺利进入early stop后保存的模型
        #tokenizer = tokenizer_class.from_pretrained(final_dir, do_lower_case=args.do_lower_case)
        model = model_class.from_pretrained(early_stop_dir,#early_stop_dir,=>顺利进入early stop而保存的模型
                                            args=args,
                                            config_class=config_class,
                                            model_class=base_model,
                                            )
