...
def main():
    ...
    parser.add_argument("--log_file", type=str, default='POS-halflog.txt', help="log file")
    ...
    lora_config =LoraConfig(
        r=8,#越大性能越好，但是速度也会随之减慢
        lora_alpha=16,#缩放因子，会是r的4倍
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        logger.info("Loading the best checkpoint from {}\n".format(early_stop_dir))
        tokenizer = tokenizer_class.from_pretrained(early_stop_dir, do_lower_case=args.do_lower_case)
        model = model_class.from_pretrained(early_stop_dir,
                                            args=args,
                                            config_class=config_class,
                                            model_class=base_model)
        model.to(args.device)
