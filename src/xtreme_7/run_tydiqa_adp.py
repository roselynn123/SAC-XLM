...
emb_config = {"mbert": 768, "xlmr": 1024, "infoxlm": 1024}
...
def train(args, train_dataset, model, tokenizer):
    ...
    def optimizer_schedule(model):
        ...
        # 1. IA3 适配器参数 (需要更高学习率)
        ia3_params = [
            {"params": [p for n, p in model.named_parameters() if "ia3" in n and p.requires_grad], "weight_decay": 0.0, "lr": args.learning_rate * 10}, # 例如 10 倍
        ]
        
        # 2. 其他可训练参数 (QA头, Actor, Critic等)
        other_params = [
            {"params": [p for n, p in model.named_parameters() if "ia3" not in n and p.requires_grad and not any(nd in n for nd in no_decay)], "weight_decay": args.weight_decay},
            {"params": [p for n, p in model.named_parameters() if "ia3" not in n and p.requires_grad and any(nd in n for nd in no_decay)], "weight_decay": 0.0},
        ]
        
        # 合并所有参数组
        optimizer_grouped_parameters = ia3_params + other_params
        '''
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer, num_warmup_steps=t_total * args.warmup_steps, num_training_steps=t_total
        )
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    if args.save_only_best_checkpoint:
                        ...
                        if results["f1"] > best_score:
                            for key, value in results.items():
                                tb_writer.add_scalar("eval_{}".format(key), value, global_step)
                            logger.info(" result['F1']={} > best_score={}".format(results["f1"], best_score))
                            logger.info(" output1 Loss: {}".format(loss))
                            ...
                            if args.use_adapter:
                                # --- Adapter 微调模式下，保存 Adapter 和其他组件 ---
                                try:
                                    # PEFT 模型使用 save_pretrained 保存 adapter 权重
                                    model_to_save.save_pretrained(output_dir)
                                    logger.info("Saving adapter checkpoint to %s", output_dir)
                                ...
                                # 保存其他自定义模块
                                if hasattr(model_to_save, 'critic'):
                                    torch.save(model_to_save.critic.state_dict(), 
                                            os.path.join(output_dir, "critic_model.bin"))
                                    logger.info("Saving critic model to %s", output_dir)
                                
                                if hasattr(model_to_save, 'actor'):
                                    torch.save(model_to_save.actor.policy_network.state_dict(), 
                                            os.path.join(output_dir, "actor_model.bin"))
                                    logger.info("Saving actor model to %s", output_dir)
...
def evaluate(args, model, tokenizer, prefix="", language="en", lang2id=None, dataset_name="squad"):
    dataset, examples, features = load_and_cache_examples(
        args, tokenizer, evaluate=True, output_examples=True, language=language, lang2id=lang2id
    )
    model.eval()
...
def main():
    ...
    parser.add_argument(
        "--critic_output_dir", default="/data/home10b/wlj/code/Struct-XLM/src/Struct_XLM/output_new/checkpoint-best", type=str
    )
    ...
    # --- 新增 Adapter 微调参数 ---
    parser.add_argument(
        "--use_adapter", action="store_true", help="Whether to use adapter-based fine-tuning."
    )
    parser.add_argument("--adapter_name", type=str, default="tydiqa_en", help="Name of the adapter to be trained.")
    parser.add_argument(
        "--adapter_type",
        type=str,
        default="houlsby",
        help="Adapter type (e.g., 'houlsby', 'pfeiffer').",
    )
    parser.add_argument(
    ...
    if args.use_adapter:
        ...
        from peft.tuners.ia3.config import IA3Config
        from peft.utils.peft_types import TaskType
        
        if args.do_train:
            
           # 直接使用 IA3Config 配置适配器（原生支持 target_modules）
            ia3_config = IA3Config(
                #task_type=TaskType.QUESTION_ANSWERING,  # 任务类型
                task_type="QUESTION_ANS",
                target_modules=[  # 直接指定目标模块（与模型结构匹配）
                    "query",
                    "key",
                    "value",
                    "output.dense",
                ...
                feedforward_modules=["intermediate.dense"],  # 前馈模块
                inference_mode=False,  # 训练模式
            )
            
            # 创建带 IA3 适配器的模型
            model = get_peft_model(model, ia3_config)
            logger.info(f"初始化 IA3 适配器，目标模块：{ia3_config.target_modules}")

        elif args.do_eval or args.do_predict:
            # 加载已训练的适配器
            adapter_path = os.path.join(args.output_dir, args.adapter_name)
            if os.path.exists(adapter_path):
                model = PeftModel.from_pretrained(model, adapter_path)
                logger.info(f"从 {adapter_path} 加载适配器")
            else:
                logger.warning(f"未找到适配器文件 {adapter_path}，将使用基础模型")
    ...
    if args.use_adapter:
        for name, param in model.named_parameters():
            # 冻结预训练模型主体参数（仅保留适配器和顶层参数可训练）
            if "adapter" not in name and "qa_outputs" not in name and "actor" not in name and "critic" not in name:
    ...
    # 遍历 model.parameters()，在 Adapter 模式下会自动跳过冻结的参数
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # 在 Adapter 微调模式下，保存 PEFT 模型
        if args.use_adapter:
            model_to_save = model.module if hasattr(model, "module") else model
            # PEFT 使用 save_pretrained 而不是 save_adapter
            model_to_save.save_pretrained(os.path.join(args.output_dir, args.adapter_name))
            tokenizer.save_pretrained(os.path.join(args.output_dir, args.adapter_name))
            logger.info("Saving adapter to %s", os.path.join(args.output_dir, args.adapter_name))
        else:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            if args.use_adapter:
                ...
                eval_checkpoint_path = os.path.join(args.output_dir, "checkpoint-best")
                base_model = model_class.from_pretrained(args.model_name_or_path)
                # 使用 PeftModel.from_pretrained 加载 adapter
                adapter_path = os.path.join(args.output_dir, args.adapter_name)
                model_for_eval = PeftModel.from_pretrained(base_model, adapter_path)
                model_for_eval.to(args.device)
                
                result = evaluate(args, model_for_eval, tokenizer, prefix="", language=args.eval_lang, lang2id=lang2id)
                results.update(result)
            else:
                # 原始加载方式
                base_model = model_class.model_class.from_pretrained(checkpoint, force_download=True,
                                                    args=args,
                                                    config_class=config_class,
                                                    model_class=base_model, )
                # 注意：这里应该是 adapter-transformers 的 API，但你用的是 PEFT
                # 如果是 adapter-transformers，才用 load_adapter
                try:
                    base_model.load_adapter(os.path.join(args.output_dir, args.adapter_name))
                    base_model.set_active_adapter(args.adapter_name)
                except:
                    pass  # 如果失败就用基础模型
                base_model.to(args.device)
                result = evaluate(args, base_model, tokenizer, prefix="", language=args.eval_lang, lang2id=lang2id)
                results.update(result)
        '''
        for checkpoint in checkpoints:
            if args.use_adapter:
                ...
                # 1. 加载基础模型 (使用与训练时相同的参数)
                try:
                    # 尝试加载检查点路径的基础模型部分（如果是完整保存的）
                    base_model = model_class.from_pretrained(
                        checkpoint,
                        args=args,
                        config_class=config_class,
                        model_class=base_model, # 这里应该是 MODEL_CLASSES 返回的原始 transformers 模型类
                        config=config, # 确保 config 已定义
                        from_tf=bool(".ckpt" in checkpoint),
                        cache_dir=args.cache_dir if args.cache_dir else None,
                    )
                    logger.info(f"Loaded base model from checkpoint: {checkpoint}")
                    adapter_load_path = checkpoint # Adapter 就在同一个目录
                except Exception as e1:
                    logger.warning(f"Failed to load base model from {checkpoint} as full model: {e1}")
                    try:
                        # 备选方案：从预训练路径加载基础模型
                        base_model = model_class.from_pretrained(
                            args.model_name_or_path, # 使用预训练模型路径
                            args=args,
                            config_class=config_class,
                            model_class=base_model, # 这里应该是 MODEL_CLASSES 返回的原始 transformers 模型类
                            config=config,
                        ...
                        logger.info(f"Loaded base model from pre-trained: {args.model_name_or_path}")
                        adapter_load_path = checkpoint # Adapter 从指定的检查点目录加载
                ...
                # 2. 加载适配器权重
                try:
                    model_for_eval = PeftModel.from_pretrained(base_model, adapter_load_path)
                    model_for_eval.to(args.device)
                    logger.info(f"Successfully loaded PEFT adapter from {adapter_load_path}")
                    
                    result = evaluate(args, model_for_eval, tokenizer, prefix=f"checkpoint_{os.path.basename(checkpoint)}", language=args.eval_lang, lang2id=lang2id)
                    results.update({f"{os.path.basename(checkpoint)}_{k}": v for k, v in result.items()}) # 为结果键加上检查点前缀
                    
                except Exception as e:
                    logger.error(f"Failed to load adapter from {adapter_load_path}: {e}")
                    # 可以选择跳过或使用基础模型评估
                    # model_for_eval = base_model.to(args.device)
                    # result = evaluate(args, model_for_eval, tokenizer, prefix=f"base_{os.path.basename(checkpoint)}", language=args.eval_lang, lang2id=lang2id)
                    # results.update({f"base_{os.path.basename(checkpoint)}_{k}": v for k, v in result.items()})
                    
            else:
                # 全参数微调模型加载方式
                try:
                    model_for_eval = model_class.from_pretrained(
                        checkpoint,
                        args=args,
                        config_class=config_class,
                        model_class=base_model,
                    ...
                    model_for_eval.to(args.device)
                    result = evaluate(args, model_for_eval, tokenizer, prefix=f"full_{os.path.basename(checkpoint)}", language=args.eval_lang, lang2id=lang2id)
                    results.update({f"full_{os.path.basename(checkpoint)}_{k}": v for k, v in result.items()})
                except Exception as e:
                    logger.error(f"Failed to load full model from {checkpoint}: {e}")
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if args.use_adapter:
            ...
            adapter_path = os.path.join(best_checkpoint, args.adapter_name)
            if os.path.exists(adapter_path):
                model = PeftModel.from_pretrained(base_model, adapter_path)
                logger.info(f"Loaded adapter from {adapter_path}")
            else:
                logger.warning(f"Adapter not found at {adapter_path}. Predicting with base model.")
                model = base_model
        else:
            model = model_class.from_pretrained(
                best_checkpoint,
        ...
        if args.do_train:
            ...
            # 否则使用指定的模型路径
            best_checkpoint = args.model_name_or_path
        ...
        if args.use_adapter:
            ...
            try:
                ...
                logger.info(f"Loaded base model for prediction from: {args.model_name_or_path}")
            except Exception as e:
                logger.error(f"Failed to load base model for prediction: {e}")
                raise e

            # 2. 加载适配器
            adapter_path = os.path.join(best_checkpoint, args.adapter_name) if not os.path.isfile(os.path.join(best_checkpoint, "adapter_model.bin")) else best_checkpoint
            if os.path.exists(adapter_path) and any(f.startswith("adapter") for f in os.listdir(adapter_path)):
                try:
                    model = PeftModel.from_pretrained(base_model, adapter_path)
                    logger.info(f"Loaded adapter from {adapter_path}")
                except Exception as e:
                    logger.error(f"Failed to load adapter from {adapter_path}: {e}")
                    logger.warning("Predicting with base model only.")
                    model = base_model
            else:
                # 检查 best_checkpoint 目录本身
                if os.path.exists(best_checkpoint) and any(f.startswith("adapter") for f in os.listdir(best_checkpoint)):
                    try:
                        model = PeftModel.from_pretrained(base_model, best_checkpoint)
                        logger.info(f"Loaded adapter from {best_checkpoint}")
                    except Exception as e:
                        logger.error(f"Failed to load adapter from {best_checkpoint}: {e}")
                        logger.warning("Predicting with base model only.")
                        model = base_model
                else:
                    logger.warning(f"Adapter not found at {adapter_path} or {best_checkpoint}. Predicting with base model.")
                    model = base_model
        else:
            # 全参数微调模型预测加载方式
            model = model_class.from_pretrained(
                ...
                model_class=base_model,
                config=config,
                from_tf=bool(".ckpt" in best_checkpoint),
                cache_dir=args.cache_dir if args.cache_dir else None,
            )
        ...
        if "tydiqa" in args.test_file:
            ...
            for lang in [
                ...
                result = evaluate(args, model, tokenizer, prefix=u"mlqa {}".format(lang), language=lang, lang2id=lang2id)
